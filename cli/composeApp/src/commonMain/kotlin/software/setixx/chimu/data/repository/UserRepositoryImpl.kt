...
class UserRepositoryImpl(
    ...
    override suspend fun changePassword(body: ChangePassword): ApiResult<ChangedPassword> {
        return try {
    ...
    override suspend fun getUserById(userId: String): ApiResult<PublicUserProfile> {
        return try {
            val token = tokenStorage.getAccessToken()
                ?: return ApiResult.Error("Ошибка аутентификации")
    ...
    private fun PublicUserProfileResponse.toDomain(): PublicUserProfile {
        if (isDeleted) {
